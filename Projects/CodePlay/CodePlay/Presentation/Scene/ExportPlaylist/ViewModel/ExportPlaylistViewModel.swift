...
protocol ExportPlaylistViewModel {
    func preProcessRawText(_ rawText: RawText, completion: @escaping (Bool) -> Void) // 텍스트를 띄어쓰기에 맞춰서 여러개 쪼개고, 하나의 데이터로 저장하는 로직 (RawText 업데이트)
    var artistCandidates: Observable<Set<String>> { get } // 아티스트 스트링 관리, 추후 리펙토링 필요
...
final class DefaultExportPlaylistViewModel: ExportPlaylistViewModel {
    ...
    func preProcessRawText(_ rawText: RawText, completion: @escaping (Bool) -> Void) {
