...
// MARK: MusicSetupViewModelInput
protocol LicenseCheckViewModelInput {
    /// 음악 권한을 요청하는 함수
...
// MARK: MusicSetupViewModelOutput
protocol LicenseCheckViewModelOutput {
...
// MARK: MusicSetupViewModel
protocol LicenseCheckViewModel: LicenseCheckViewModelInput,
    LicenseCheckViewModelOutput, ObservableObject
{}

// MARK: DefaultMusicSetupViewModel
final class DefaultLicenseCheckViewModel: LicenseCheckViewModel {
    ...
    func requestMusicAuthorization() {
        print("🎵 권한 요청 시작")
                ...
                == "1"
            {
                ...
                let mockStatus = MusicAuthorizationStatusModel(
                    from: .authorized
                )
                let mockSubscription = MusicSubscriptionModel(
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    let statusModel = MusicAuthorizationStatusModel(
                        from: status
                    )
    ...
    func checkMusicSubscription() {
        ...
        // 구독 확인 중 상태로 설정
        let checkingSubscription = MusicSubscriptionModel(
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    let subscriptionModel = MusicSubscriptionModel(
                        hasActiveSubscription: canPlay
                    )
                ...
                let errorMsg =
                    "구독 상태를 확인할 수 없습니다: \(error.localizedDescription)"
